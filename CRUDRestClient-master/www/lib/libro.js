...
$.libro.HOST = 'http://localhost:3001';

$.libro.URL = '/Gestion_Biblioteca/webresources/entidades.libro';
...
$.libro.LibroReadREST = function(id) {
    if ( id === undefined ) {
            ...
            success: function (json) {
                ...
                for (var clave in json) {
                    tbody.append($('<tr />').append('<td>' + json[clave].id + '</td>',
                                '<td>' + json[clave].nombre + '</td>', '<td>' + json[clave].codigoISBN + '</td>', '<td>' + json[clave].autor + '</td>','<td>' + new Date (json[clave].fechaPretamo).toDateString() + '</td>','<td>' +  new Date (json[clave].fechaDevolucion).toDateString() + '</td>'));
                }
                table.append(tbody);
                console.log(json[0]);
        ...
        $.ajax({
            url: 'http://localhost:3001/Gestion_Biblioteca/webresources/entidades.libro',
            type: 'GET',
            dataType: 'json',
            success: function (json) {
...
'fechaPretamo': $('#c_li_FechaPrestamo').val()+"T00:00:00+02:00", 'nombre':$("#c_li_nombre").val()
                  };
    ...
    if ( id === undefined ) {
        $.ajax({
            ...
            success: function (json) {
                ...
                table.append($('<thead />').append($('<tr />').append('<th>id</th>', '<th>Titulo</th>', '<th>ISBN</th>', '<th>Autor</th>')));
                var tbody = $('<tbody />');
                for (var clave in json) {
        ...
        var li_fechaPretamo = ($(seleccion))[4];
        ...
        $("#u_li_fechaPretamo").val(li_fechaPretamo.childNodes[0].data);
        ...
        //HACEMOS LA LLAMADA REST
            var datos = {
                ...
                'codigoISBN':$("#u_li_codigoISBN").val(),
                'autor': $("#u_li_autor").val(), 'fechaPretamo':$('#u_li_fechaPretamo').val()+"T00:00:00+02:00", 'fechaDevolucion':$('#u_li_fechaDevolucion').val()+"T00:00:00+02:00"
            };

            // comprobamos que en el formulario haya datos...
            if ( datos.nombre.length>2 && datos.codigoISBN.length>2 && datos.autor.length>2 &&
               datos.fechaPretamo.length>2 && datos.fechaDevolucion.length>2) {
                $.ajax({
